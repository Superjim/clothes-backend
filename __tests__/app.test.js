			...
			return request(app)
				...
				.then((response) => {
					...
					for (let i = 0; i < clothesArray.length; i++) {
						expect(clothesArray[i]).toHaveProperty("clothes_id", expect.any(Number));
						...
						expect(clothesArray[i]).toHaveProperty("category", expect.any(String));
						...
						expect(clothesArray[i]).toHaveProperty("item_img_url", expect.any(String));
					}
				});
		});
	});
				...
				.then(() => {
					return request(app)
						...
						.then((response) => {
							const user = response.body.user;
							expect(user).toHaveProperty("uid", "12342341");
							expect(user).toHaveProperty("preferences", "red green blue yellow");
						});
				});
		});
			...
			return request(app)
				...
				.then(({ body }) => {
					...
					for (let i = 0; i < suggestedArr.length; i++) {
						...
						expect(suggestedArr[i]).toHaveProperty("clothes_id", expect.any(Number));
						...
						expect(suggestedArr[i]).toHaveProperty("category", expect.any(String));
						...
						expect(suggestedArr[i]).toHaveProperty("item_img_url", expect.any(String));
					}
				});
		});
			...
			return request(app)
				...
				.then((response) => {
					...
					for (let i = 0; i < favouritesArr.length; i++) {
						...
						expect(favouritesArr[i]).toHaveProperty("favourite_id", expect.any(Number));
						expect(favouritesArr[i]).toHaveProperty("uid", expect.any(String));
						expect(favouritesArr[i]).toHaveProperty("clothes_id", expect.any(Number));
					}
				});
		});
